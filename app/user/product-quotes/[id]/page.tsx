...
import { getProductQuoteDetail } from "@/apiServices/user";
import { useUserInfo } from '@/lib/useUserInfo';
import { toast } from 'sonner';

const ProductQuoteDetailPage = () => {
  ...
  useEffect(() => {
    const fetchDetail = async () => {
      ...
      try {
        const response = await getProductQuoteDetail(params.id);
        if (response.success) {
          setQuoteDetail(response.data);
        } else {
          setError(response.message || "Failed to fetch product quote details");
          toast.error("Failed to fetch product quote details");
        }
      } catch (err) {
        console.error("Error fetching product quote detail:", err);
        setError("An error occurred while fetching product quote details");
        toast.error("An error occurred while fetching product quote details");
      } finally {
        setLoading(false);
      }
    };
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-50 via-green-50/20 to-emerald-50/30 flex items-center justify-center">
        <div className="text-center">
          ...
          <h3 className="text-lg font-semibold text-gray-900 mb-2">Loading Product Quote Details...</h3>
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-50 via-green-50/20 to-emerald-50/30 flex items-center justify-center">
        <div className="text-center">
          ...
          <button
            onClick={() => router.push('/user/product-quotes')}
          ...
          <div className="xl:col-span-2 space-y-8">
            ...
            <div className="bg-white/80 backdrop-blur-sm rounded-2xl shadow-lg border border-gray-200/50 p-8">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div className="bg-gray-50 rounded-xl p-4">
                      ...
                      <p className="font-semibold text-gray-900">
                        {quoteDetail.product?.chemicalName || 'N/A'}
                      </p>
                    </div>
                    <div className="bg-gray-50 rounded-xl p-4">
                      <p className="text-sm text-gray-600 mb-1">Country of Origin</p>
                      <p className="font-semibold text-gray-900">
                        {quoteDetail.product?.countryOfOrigin || 'N/A'}
                      </p>
                    </div>
                    <div className="bg-gray-50 rounded-xl p-4">
                      <p className="text-sm text-gray-600 mb-1">Grade</p>
                      <p className="font-semibold text-gray-900">
                        {quoteDetail.orderDetails?.grade?.name || quoteDetail.orderDetails?.grade || 'N/A'}
                      </p>
                    </div>
                    <div className="bg-gray-50 rounded-xl p-4">
                      <p className="text-sm text-gray-600 mb-1">Color</p>
                      <p className="font-semibold text-gray-900">
                        {quoteDetail.product?.color || 'N/A'}
                      </p>
                    </div>
                  </div>
                </div>
                ...
                <div className="space-y-4">
                  {/* Product Images */}
                  {quoteDetail.product?.productImages && quoteDetail.product.productImages.length > 0 && (
                    <div className="bg-gradient-to-br from-orange-50 to-amber-50 rounded-xl p-6 border border-orange-200/50">
                      ...
                      <div className="grid grid-cols-2 gap-3">
                        {quoteDetail.product.productImages.slice(0, 4).map((image: { fileUrl: string; fileName: string }, idx: number) => (
                      ...
                      {quoteDetail.product.productImages.length > 4 && (
                        <p className="text-xs text-orange-600 mt-2">
                          +{quoteDetail.product.productImages.length - 4} more images
                        </p>
                      )}
                    </div>
                  )}
                ...
                <div className="bg-gray-50 rounded-xl p-4">
                  ...
                  <p className="font-semibold text-gray-900">
                    {quoteDetail.orderDetails?.deliveryDate ? 
                      new Date(quoteDetail.orderDetails.deliveryDate).toLocaleDateString('en-US', {
                        year: 'numeric',
                        month: 'short',
                        day: 'numeric'
                      }) : 'N/A'
            ...
            {/* Shipping Terms */}
            {quoteDetail.orderDetails?.incoterm && (
              <div className="bg-white/80 backdrop-blur-sm rounded-2xl shadow-lg border border-gray-200/50 p-8">
                <div className="flex items-center gap-3 mb-6">
                ...
                <div className="bg-gradient-to-br from-purple-50 to-indigo-50 rounded-xl p-6 border border-purple-200/50">
                  ...
                  <p className="font-bold text-xl text-purple-900">
                    {quoteDetail.orderDetails.incoterm?.name || quoteDetail.orderDetails.incoterm}
                  </p>
                </div>
              </div>
            )}
...
export default ProductQuoteDetailPage;
