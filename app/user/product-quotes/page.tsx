...
import { getProductQuoteRequests } from "@/apiServices/user";
...
const ProductQuotesPage = () => {
  ...
  const fetchProductQuotes = async () => {
    ...
    try {
      ...
      const response = await getProductQuoteRequests(params);
      
      if (response.success) {
        setProductQuotes(response.data || []);
        ...
        setError(response.message || "Failed to fetch product quotes");
        toast.error("Failed to fetch product quotes");
      }
    } catch (err) {
      console.error("Error fetching product quotes:", err);
      setError("An error occurred while fetching product quotes");
      toast.error("An error occurred while fetching product quotes");
    } finally {
      setLoading(false);
    }
  };
  ...
  const getStatsData = () => {
    return [
      { label: 'Total Requests', value: totalItems, icon: FileText, color: 'text-green-600', bgColor: 'bg-green-50' },
            ...
            <div className="flex-1">
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full pl-12 pr-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-green-500 focus:border-transparent transition-all duration-200 bg-white"
                />
              </div>
            </div>
            ...
            <div className="flex gap-4">
              <div className="relative min-w-[200px]">
                ...
                <select
                  ...
                  className="w-full pl-12 pr-10 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-green-500 focus:border-transparent transition-all duration-200 appearance-none bg-white cursor-pointer"
                >
        ...
        <div className="bg-white/80 backdrop-blur-sm rounded-2xl shadow-lg border border-gray-200/50 overflow-hidden">
          {loading ? (
            ...
            <div className="text-center py-20">
              ...
              <h3 className="text-lg font-semibold text-gray-900 mb-2">No Product Quotes Found</h3>
              ...
              <Table>
                ...
                <TableBody>
                  {productQuotes.map((quote) => (
                    <TableRow key={quote._id} className="hover:bg-green-50/30 transition-colors">
                      <TableCell>
                        <div className="flex items-center gap-3">
                          ...
                          <div className="flex flex-col">
                            <span className="font-semibold text-gray-900">{quote.product?.productName || 'N/A'}</span>
                            <span className="text-xs text-gray-500">{quote.product?.chemicalName || ''}</span>
                          </div>
                        </div>
                      </TableCell>
                      <TableCell>
                        <div className="flex flex-col gap-1">
                          <span className="font-medium text-gray-900">{quote.buyer?.name || 'N/A'}</span>
                          <span className="text-sm text-gray-600">{quote.buyer?.company || 'N/A'}</span>
                        </div>
                      </TableCell>
                      <TableCell>
                        <span className="font-medium text-gray-900">
                          {quote.orderDetails?.quantity || 0} {quote.orderDetails?.uom || 'units'}
                        </span>
                      </TableCell>
                      <TableCell>
                        <div className="flex items-center gap-2">
                          ...
                          <div className="flex flex-col">
                            <span className="text-sm text-gray-900">{quote.orderDetails?.destination || 'N/A'}</span>
                            <span className="text-xs text-gray-500">{quote.orderDetails?.country || ''}</span>
                          </div>
                        </div>
                      </TableCell>
                      <TableCell>
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-sm text-gray-700">
                            {quote.orderDetails?.deliveryDate
                              ? new Date(quote.orderDetails.deliveryDate).toLocaleDateString('en-US', {
                                  year: 'numeric',
                                  month: 'short',
                                  day: 'numeric'
                                })
                      ...
                      <TableCell>
                        <button
                          onClick={() => router.push(`/user/product-quotes/${quote._id}`)}
