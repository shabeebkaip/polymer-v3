  ...
  return (
    <div className="bg-white rounded-xl shadow border hover:shadow-xl transition-all duration-200 overflow-hidden group">
      ...
      <div className="p-5">
        ...
        {/* Buyer Info */}
        <div className="flex items-center gap-3 mb-3 p-2 bg-primary-50 rounded-lg">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2">
              ...
              {request.buyer.verified && (
                <div className="bg-primary-50 text-primary-600 px-2 py-1 rounded text-xs font-bold border border-primary-500">
                  ✓
                </div>
              )}
            </div>
        ...
        <div className="grid grid-cols-2 gap-3 mb-3">
          <div className="bg-gray-50 p-3 rounded-lg">
            ...
            <p className="font-bold text-primary-600 text-sm">{request.quantity}</p>
          </div>

          <div className="bg-primary-50 p-3 rounded-lg">
            <div className="flex items-center gap-2 mb-1">
              <Clock className="w-3 h-3 text-primary-500" />
              <span className="text-xs text-primary-500 uppercase font-semibold">Deadline</span>
            </div>
            <p className="font-bold text-primary-600 text-sm">{formatDate(request.deadline)}</p>
          </div>
        </div>
        ...
        {request.destination && (
          <div className="bg-primary-50 p-3 rounded-lg mb-3">
            <div className="flex items-center gap-2 mb-1">
              ...
              <span className="text-xs text-primary-500 uppercase font-semibold">Destination</span>
            </div>
            <p className="font-semibold text-primary-600 text-sm">{request.destination}</p>
          </div>
        )}
        ...
        <div className="flex items-center justify-between text-xs text-gray-600 mb-4">
          ...
          {request.budget && request.budget !== 'Contact for quote' && (
            <div className="font-semibold text-primary-600">{request.budget}</div>
          )}
        </div>
